 ...
 * Route: /products
 * Purpose: Display and manage products list with filters
 */
import {
  Card,
  CardBody,
...
const Products = () => {
  ...
  // ✅ מחיקה מיידית – פריט בודד או מרובים
  const handleDeleteSelected = async (ids = null) => {
    try {
      ...
      // ריקון הבחירות
      setIsCheck(prev => prev.filter(id => !idsToDelete.includes(id)));
    } catch (err) {
      console.error(err);
    }
  };
        ...
        <>
          <TableContainer className="mb-8 rounded-b-lg hidden md:block">
            <StandardTable>
              ...
              <ProductsTable products={products} isCheck={isCheck} setIsCheck={setIsCheck} isMobile={false} handleDeleteSelected={handleDeleteSelected} />
            </StandardTable>
            <TableFooter>
