...
const LEGEND_SECTIONS = [
  ...
  },
  {
    title: "לווים",
    items: [
      { key: "{borrowerName}", label: "שם לווה מלא — שם פרטי ושם משפחה יחד (של הקובץ הנוכחי)" },
      { key: "{borrowerFirstName}", label: "שם פרטי לווה בלבד (של הקובץ הנוכחי)" },
      { key: "{borrowerLastName}", label: "שם משפחה לווה (של הקובץ הנוכחי)" },
      { key: "{borrowerIdNumber}", label: "מספר תעודת זהות לווה (כולל אפס מוביל כפי שנשמר)" },
      ...
      { key: "{borrowerIsMortgagor}", label: "ממשכן (כן / לא) — ללווה של הקובץ הנוכחי בלבד" },
      {
        key: "{mortgagorIdNumber}",
        label:
          "ממשכנים — ת.ז. של הממשכן הראשון (בלי ספרה בסוף המפתח). כולל אפס מוביל כפי שנשמר. דורש צ׳קבוקס «ממשכן»",
      },
      {
        key: "{mortgagorIdNumber1}",
        label:
          "ממשכנים — ת.ז. של הממשכן הראשון ברשימת הממשכנים (כולל אפס מוביל); {mortgagorIdNumber2} … {mortgagorIdNumber5} לפי הסדר באותה קבוצה",
      },
      {
        key: "{nonMortgagorIdNumber}",
        label:
          "לא ממשכנים: ת.ז. של הלא־ממשכן הראשון (בלי ספרה בסוף המפתח). כולל אפס מוביל כפי שנשמר",
      },
      { key: "{borrowerName1} … {borrowerName5}", label: "כל הלווים — שם מלא (פרטי + משפחה) לפי סדר" },
      { key: "{borrowerFirstName1} … {borrowerFirstName5}", label: "כל הלווים — שם פרטי בלבד לפי סדר" },
      { key: "{borrowerLastName1} … {borrowerLastName5}", label: "כל הלווים — שם משפחה בנפרד" },
      { key: "{borrowerIdNumber1} … {borrowerIdNumber5}", label: "כל הלווים — מספר ת.ז. לפי סדר בתיק (כולל אפס מוביל כפי שנשמר)" },
      { key: "{mortgagorName1} … {mortgagorName5}", label: "רק ממשכנים — שם מלא (פרטי + משפחה) לפי סדר ממשכנים" },
      ...
      { key: "{mortgagorIdNumber1} … {mortgagorIdNumber5}", label: "רק ממשכנים — מספר ת.ז. לפי סדר ממשכנים (כולל אפס מוביל)" },
      { key: "{nonMortgagorName1} … {nonMortgagorName5}", label: "רק לא ממשכנים — שם מלא (פרטי + משפחה) לפי סדר" },
      ...
      { key: "{nonMortgagorIdNumber1} … {nonMortgagorIdNumber5}", label: "לא ממשכנים — מספר ת.ז. לפי סדר לא־ממשכנים (כולל אפס מוביל)" },
